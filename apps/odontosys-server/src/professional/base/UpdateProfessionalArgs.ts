/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { ProfessionalWhereUniqueInput } from "./ProfessionalWhereUniqueInput";
import { ValidateNested } from "class-validator";
import { Type } from "class-transformer";
import { ProfessionalUpdateInput } from "./ProfessionalUpdateInput";

@ArgsType()
class UpdateProfessionalArgs {
  @ApiProperty({
    required: true,
    type: () => ProfessionalWhereUniqueInput,
  })
  @ValidateNested()
  @Type(() => ProfessionalWhereUniqueInput)
  @Field(() => ProfessionalWhereUniqueInput, { nullable: false })
  where!: ProfessionalWhereUniqueInput;

  @ApiProperty({
    required: true,
    type: () => ProfessionalUpdateInput,
  })
  @ValidateNested()
  @Type(() => ProfessionalUpdateInput)
  @Field(() => ProfessionalUpdateInput, { nullable: false })
  data!: ProfessionalUpdateInput;
}

export { UpdateProfessionalArgs as UpdateProfessionalArgs };
